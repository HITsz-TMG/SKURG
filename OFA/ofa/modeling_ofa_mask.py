...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    if len(mask.size()) == 2:
        ...
        bsz, tgt_len_enc, src_len = mask.size()
        assert tgt_len == tgt_len_enc
...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def get_patch_images_info(self, patch_images, sample_patch_num, device):
        ...
        image_padding_mask = patch_images.new_ones((patch_images.size(0), image_num_patches)).bool()
    ...
    def forward_embedding(
            ...
            token_type_embedding: Optional[torch.Tensor] = None,
    ):
        r"""
        ...
        if self.entangle_position_embedding and pos_embed is not None:
            x += pos_embed
        if token_type_embedding is not None:
            x += token_type_embedding
        elif self.type_embedding is not None:
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            encoder_padding_mask = input_ids.ne(self.padding_idx)
        if patch_images is not None:
        ...
        x, encoder_embedding = self.forward_embedding(
            ...
            pos_embed, image_pos_embed, image_pos_embed_2, token_type_embedding=type_embeddings
        )
        if has_pads:
            x = x * encoder_padding_mask.unsqueeze(-1).type_as(x)
