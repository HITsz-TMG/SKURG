...
class generative_source2cell_mmqa_dataset(Dataset):
    def __init__(self, tokenizer, input_example_dir, text_dir, img_dir, table_dir, doc_stride,
                 max_query_len, max_seq_len, connect_path, span_dir, extra_table_dir, extra_connect_path,
                 extra_span_dir, is_test=False):
        super().__init__()
        ...
        self.extra_connect = pickle.load(open(extra_connect_path, 'rb'))
        self.extra_cell_span = pickle.load(open(extra_span_dir, 'rb'))
        self.extra_tables = pickle.load(open(extra_table_dir, 'rb'))
    ...
    def example_process(self, input_data):
        ...
        for example in tqdm(input_data):
            ...
            table_context = self.table2id(self.tables[example["metadata"]["table_id"]], ques_ids)
            image_context = self.image2id(example["metadata"]["image_doc_ids"], ques_ids)
            extra_table_context = self.table2id(self.extra_tables[qid], ques_ids)
            ...
            extra_connect_info = self.extra_connect[qid].copy()
            extra_table_connect_span = []
            extra_table_connect_index = []
            for info in extra_connect_info:
                span = [item + ques_len + 2 for item in info['span']]
                span[1] = span[1] - 1
                if span[1] >= self.max_seq_len - 1:
                    # 超长
                ...
                for sc in info['connect']['image']:
                    try:
                        ...
                        continue
                if len(sc_index) != 0:
                    extra_table_connect_span.append(torch.tensor(span))
                    extra_table_connect_index.append(torch.tensor(sc_index))
            extra_table_cell_span = []
            extra_cell_span_info = self.extra_cell_span[qid].copy()
            for info in extra_cell_span_info:
                span = [item + ques_len + 2 for item in info]
                span[1] = span[1] - 1
                if span[1] >= self.max_seq_len - 1:
                    # 超长
                    break
                extra_table_cell_span.append(torch.tensor(span))
            ...
            table_context['ID'] = example["metadata"]["table_id"]
            ...
            input = {
                ...
                "extra_table_context": extra_table_context,
                ...
                'extra_table_connect_span': extra_table_connect_span,
                'extra_table_connect_index': extra_table_connect_index,
                'extra_table_cell_span': extra_table_cell_span,
            }
    ...
    def __getitem__(self, i):
        ...
        images_keys = [key for key in image_context_dict.keys()]
        ...
        table_key = table_context_dict['ID']
        ...
        extra_table_context_dict = input['extra_table_context']
        extra_table_text = extra_table_context_dict['ids']
        extra_table_mask = extra_table_context_dict['structure_mask']
        extra_table_one_mask = extra_table_context_dict['mask']
        extra_table_token_type = extra_table_context_dict['token_type']
        extra_table_connect_span = input['extra_table_connect_span']
        if len(extra_table_connect_span) == 0:
            extra_table_connect_span = [torch.tensor([0, 0])]
        extra_table_connect_span = torch.stack(extra_table_connect_span)
        extra_table_connect_index = input['extra_table_connect_index']
        if len(extra_table_connect_index) == 0:
            extra_table_connect_index = [torch.tensor([-1])]
        extra_table_connect_index = pad_sequence(extra_table_connect_index, batch_first=True, padding_value=-1)
        extra_table_cell_span = input['extra_table_cell_span']
        if len(extra_table_cell_span) == 0:
            extra_table_cell_span = [torch.tensor([0, 1])]
        extra_gather_index = torch.zeros_like(extra_table_text)
        extra_span_lable = torch.zeros((input_text.size(0) + images.size(0)), dtype=torch.long)
        for idx, span in enumerate(extra_table_cell_span):
            extra_gather_index[span[0]:span[1]] = idx + 1
            extra_connect_index = torch.nonzero(extra_table_connect_span == span)
            if extra_connect_index.size(0) >= 2:
                extra_connect_source_index = extra_table_connect_index[extra_connect_index[0, 0]]
                for item in extra_connect_source_index:
                    # 第0位用于冗余编码：question/ROW/；/PAD等符号
                    extra_span_lable[item] = idx + 1
        extra_table_cell_span = torch.stack(extra_table_cell_span)

        return [(input['qid'], input_text, input_mask, input_token_type, table_text, table_mask, table_one_mask,
                 table_token_type, extra_table_text, extra_table_mask, extra_table_one_mask, extra_table_token_type,
                 images, image_titles, title_masks, image_token_types, gpt_ids, attn_mask, golden_source_index,
                 ans_loss_mask, sc_loss_mask, input['answer_list'], table_connect_span, table_connect_index,
                 table_cell_span, gather_index, span_lable, extra_table_connect_span, extra_table_connect_index,
                 extra_table_cell_span, extra_gather_index, extra_span_lable, images_keys, input_text_keys, table_key)]

    def mmqa_collate(self, inputs):
        (ques_id, input_ids, input_mask, input_token_type, table_text, table_mask, table_one_mask, table_token_type,
         extra_table_text, extra_table_mask, extra_table_one_mask, extra_table_token_type, image_feat, titles,
         title_masks, image_token_types, gpt_ids, attn_mask, golden_source_index, ans_loss_mask, sc_loss_mask,
         answer_list, table_connect_span, table_connect_index, table_cell_span, gather_index, span_lable,
         extra_table_connect_span, extra_table_connect_index, extra_table_cell_span, extra_gather_index,
         extra_span_lable, images_keys, input_text_keys, table_key) = map(list, unzip(concat(inputs)))
        batch_text_len = []
        flatten_input_ids = []
        flatten_input_mask = []
        ...
        extra_table_text = pad_sequence(extra_table_text, batch_first=True, padding_value=0)
        extra_table_token_type = pad_sequence(extra_table_token_type, batch_first=True, padding_value=0)
        extra_table_one_mask = pad_sequence(extra_table_one_mask, batch_first=True, padding_value=0)
        extra_table_connect_num = [[item.size(0), item.size(1)] for item in extra_table_connect_index]
        extra_flatten_connect_spans = []
        extra_flatten_connect_index = []
        for idx, item in enumerate(extra_table_connect_index):
            item = item.reshape(-1)
            extra_flatten_connect_index.append(item)
            extra_flatten_connect_spans.append(extra_table_connect_span[idx].reshape(-1))
        extra_table_connect_num = torch.tensor(extra_table_connect_num)
        extra_flatten_connect_index = pad_sequence(extra_flatten_connect_index, batch_first=True, padding_value=-1)
        extra_flatten_connect_spans = pad_sequence(extra_flatten_connect_spans, batch_first=True, padding_value=-1)
        extra_table_cell_num = [item.size(0) for item in extra_table_cell_span]
        extra_flatten_cell_span = []
        for idx, item in enumerate(extra_table_cell_span):
            item = item.reshape(-1)
            extra_flatten_cell_span.append(item)
        extra_table_cell_num = torch.tensor(extra_table_cell_num)
        extra_flatten_span_label = pad_sequence(extra_span_lable, batch_first=True, padding_value=0)
        extra_flatten_cell_span = pad_sequence(extra_flatten_cell_span, batch_first=True, padding_value=-1)
        extra_gather_index = pad_sequence(extra_gather_index, batch_first=True, padding_value=0)
        ...
        batch = {'ques_id': ques_id, 'input_ids': flatten_input_ids, 'input_mask': flatten_input_mask,
                 ...
                 'table_text': table_text, 'table_token_type': table_token_type,
                 ...
                 'extra_table_text': extra_table_text, 'extra_table_token_type': extra_table_token_type,
                 'extra_table_one_mask': extra_table_one_mask,
                 'extra_table_connect_num': extra_table_connect_num,
                 'extra_flatten_connect_spans': extra_flatten_connect_spans,
                 'extra_flatten_connect_index': extra_flatten_connect_index, 'extra_span_lable': extra_span_lable,
                 'extra_flatten_cell_span': extra_flatten_cell_span, 'extra_table_cell_num': extra_table_cell_num,
                 'extra_gather_index': extra_gather_index, 'extra_flatten_span_label': extra_flatten_span_label,
                 'input_text_keys': input_text_keys, 'images_keys': images_keys, 'table_key': table_key
                 }
