...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
            ...
            output_attentions: Optional[bool] = None,
            output_hidden_states: Optional[bool] = None,
            return_dict: Optional[bool] = None, gs_hidden=None, gs_index=None
        ...
        if gs_index is not None and gs_hidden is not None:
            for i in range(len(gs_hidden)):
                if type(gs_hidden[i]) == list:
                    # for retrieval beam search
                    for local_index, input_index in enumerate(gs_index[i]):
                        inputs_embeds[i, input_index] = gs_hidden[i][local_index]
                else:
                    inputs_embeds[i, gs_index[i][0]:gs_index[i][1]] = gs_hidden[i]
